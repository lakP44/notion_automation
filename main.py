...
def Run():
    ...
    write_log("logs", f"-------------------- 가공 데이터 추출 시작 --------------------", False)

    # 계획 생성표 데이터 추출
    total_create_db_result = fetch_create_plan_data(notion, create_pages)
    write_log("logs", f"가공된 계획 생성표 데이터 (종료일 설정 등등...): {total_create_db_result}", False)

    # 전체 계획 및 주간 계획 데이터 추출
    all_view_db_result, week_view_db_result = fetch_view_plan_data(view_pages)
    write_log("logs", f"가공된 전체 계획표 데이터: {all_view_db_result}", False)
    write_log("logs", f"가공된 주간 계획표 데이터: {week_view_db_result}", False)

    write_log("logs", f"-------------------- 캘린더 계획 생성 및 업데이트 시작 --------------------", False)

    # 캘린더 계획 생성 및 상태 업데이트 실행
    generate_calendar_plans(notion, total_create_db_result, week_view_db_result, all_view_db_result)

    write_log("logs", f"-------------------- 코드 전체 사이클 순회 완료 --------------------", True)
